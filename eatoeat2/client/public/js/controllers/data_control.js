...
app.controller('cook_controller', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.toggleSelection2 = function toggleSelection2(val) {
        ...
        // is newly selected
            ...
            $scope.selection2.push(val);
            console.log($scope.selection2);
            $scope.food_details.cuisine_types = $scope.selection2;
        }
    }
...
app.controller('cook_register', ['$scope', '$http','$location','$cookieStore','$timeout','$base64','$window', function ($scope, $http,$location,$cookieStore,$timeout,$base64,$window) {
...
$scope.u.update_food_details.occassion_list = $scope.selection;
$scope.u.update_food_details.cuisine_list = $scope.selection2;
$scope.u.cook_id=$cookieStore.get('cook_logged_in');
...
$scope.upload_user_profile_image = function(){

    if( document.getElementById('profile-image').files[0]==undefined)
    {
        alert('No Image Seleted')
    }
    else{

          $scope.user_id=$cookieStore.get('s3cr3t_user');
  var f = document.getElementById('profile-image').files[0],
      r = new FileReader();
      r.onloadend = function(e){
    ...
    //send your binary data via $http or $resource or do anything else with it
       ...
       $http({
                            method: "POST",
                            url: "user/user-profile-image-upload",
                             ...
                             data: {
                                   ...
                                   $scope.user_profile_image_status=true;
